...
const ITEM_ICONS = {
    'Bois': '🪵', 'Pierre': '🪨', 'Poisson': '🐟', 'Eau': '💧', 'Poisson Cuit': '🔥', 'default': '物品'
};
...
export function draw(gameState) {
    ...
    if (imageToDraw) {
        ...
        if (imageAspect > canvasAspect) { sHeight = imageToDraw.height; sWidth = sHeight * canvasAspect; sx = (imageToDraw.width - sWidth) / 2; sy = 0; }
        else { sWidth = imageToDraw.width; sHeight = sWidth / canvasAspect; sx = 0; sy = (imageToDraw.height - sHeight) / 2; }
        mainViewCtx.drawImage(imageToDraw, sx, sy, sWidth, sHeight, 0, 0, mainViewCanvas.width, mainViewCanvas.height);
    } else {
        mainViewCtx.fillStyle = 'white'; mainViewCtx.fillText(`Image de fond '${backgroundKey}' non trouvée`, 20, 40);
    }
}
...
export function drawSceneCharacters(gameState) {
    ...
    if (player.animationState) {
        ...
        characterPositions.forEach(p => { drawCharacter(charactersCtx, p.char, p.x, p.y, p.isPlayer); });
    }
}
...
export function drawLargeMap(gameState, config) {
    const { map, player, npcs } = gameState; const { MAP_WIDTH, MAP_HEIGHT } = config; const cellSize = Math.min(largeMapCanvas.parentElement.clientWidth / MAP_WIDTH, largeMapCanvas.parentElement.clientHeight / MAP_HEIGHT, 40);
    largeMapCanvas.width = MAP_WIDTH * cellSize; largeMapCanvas.height = MAP_HEIGHT * cellSize;
    largeMapCtx.clearRect(0, 0, largeMapCanvas.width, largeMapCanvas.height);
    for (let y = 0; y < MAP_HEIGHT; y++) { for (let x = 0; x < MAP_WIDTH; x++) { const tile = map[y][x]; largeMapCtx.fillStyle = tile.type.color || '#ff00ff'; largeMapCtx.fillRect(x * cellSize, y * cellSize, cellSize, cellSize); largeMapCtx.fillStyle = 'rgba(255, 255, 255, 0.8)'; largeMapCtx.font = `bold ${cellSize * 0.6}px Poppins`; largeMapCtx.textAlign = 'center'; largeMapCtx.textBaseline = 'middle'; largeMapCtx.fillText(tile.type.name.charAt(0), x * cellSize + cellSize / 2, y * cellSize + cellSize / 2); } }
    largeMapCtx.strokeStyle = 'rgba(0, 0, 0, 0.2)'; largeMapCtx.lineWidth = 1; for (let i = 0; i <= MAP_WIDTH; i++) { largeMapCtx.beginPath(); largeMapCtx.moveTo(i * cellSize, 0); largeMapCtx.lineTo(i * cellSize, largeMapCanvas.height); largeMapCtx.stroke(); } for (let i = 0; i <= MAP_HEIGHT; i++) { largeMapCtx.beginPath(); largeMapCtx.moveTo(0, i * cellSize); largeMapCtx.lineTo(largeMapCanvas.width, i * cellSize); largeMapCtx.stroke(); }
    npcs.forEach(npc => { largeMapCtx.fillStyle = npc.color; largeMapCtx.beginPath(); largeMapCtx.arc(npc.x * cellSize + cellSize / 2, npc.y * cellSize + cellSize / 2, cellSize * 0.35, 0, Math.PI * 2); largeMapCtx.fill(); });
    largeMapCtx.fillStyle = player.color; largeMapCtx.beginPath(); largeMapCtx.arc(player.x * cellSize + cellSize / 2, player.y * cellSize + cellSize / 2, cellSize * 0.4, 0, Math.PI * 2); largeMapCtx.fill(); largeMapCtx.strokeStyle = 'white'; largeMapCtx.lineWidth = 3; largeMapCtx.stroke();
}

// NOUVEAU : Fonction pour mettre à jour l'état des boutons de consommation
function updateConsumeButtons(player) {
    ...
    const canHeal = inv['Poisson Cuit'] && inv['Poisson Cuit'] > 0; // Seul le poisson cuit soigne
...
function updateStatsPanel(player) {
    ...
    // On met aussi à jour les boutons ici
...
export function updateAllButtonsState(gameState) {
    ...
    document.querySelectorAll('.nav-button-overlay').forEach(b => b.disabled = isPlayerBusy);
    actionsEl.querySelectorAll('button').forEach(b => { if (isPlayerBusy) b.disabled = true; });
    // Les boutons de consommation rapide sont aussi désactivés si le joueur est occupé
    document.querySelectorAll('.consume-btn').forEach(b => { if(isPlayerBusy) b.disabled = true; });

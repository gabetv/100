...
export function generateMap(config) {
    ...
    for (let y = 0; y < MAP_HEIGHT; y++) { for (let x = 0; x < MAP_WIDTH; x++) { const type = map[y][x].type; map[y][x] = { type, x, y, harvestsLeft: type.harvests || 0, resources: type.resource ? { type: type.resource.type, yield: type.resource.yield } : null, occupant: null }; } }
    console.log("Map generation finished."); return map;
}

...
import * as State from '../state.js'; // Pour gameState.tutorialState
...
export function showTutorialStep(stepIndex) {
    console.log("[TUTORIAL] showTutorialStep, index:", stepIndex, "Active:", State.state.tutorialState.active);
    ...
    if (!State.state.tutorialState.active || stepIndex >= tutorialSteps.length) {
    ...
    State.state.tutorialState.step = stepIndex;
}

export function advanceTutorial() {
    if (!State.state.tutorialState.active) return;
    const currentStep = State.state.tutorialState.step;
...
export function skipTutorial() {
    ...
    State.state.tutorialState.isTemporarilyHidden = false; // Assurer que c'est réinitialisé
    localStorage.setItem('tutorialCompleted', 'true');
    if (window.fullUIUpdate) window.fullUIUpdate();
}

export function completeTutorial() {
    if (!State.state.tutorialState.active && !State.state.tutorialState.completed && localStorage.getItem('tutorialCompleted') !== 'true') {
    ...
    if (window.UI && window.UI.addChatMessage && !State.state.tutorialState.welcomeMessageShown) {
        setTimeout(() => {
             ...
             State.state.tutorialState.welcomeMessageShown = true;
        }, 500);
    }
}
...
export function initTutorial() {
    ...
    State.state.tutorialState.welcomeMessageShown = false;
